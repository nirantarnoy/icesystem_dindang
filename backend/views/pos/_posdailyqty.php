                ...
                <tr>
                    ...
                    <?php for ($y = 0; $y <= count($product_header) - 1; $y++): ?>
                        ...
                        if($qty >0){
                            ...
                            $show_qty = ($car_issue_qty);
                            $sale_total_qty[$y] = ($sale_total_qty[$y] + ($car_issue_qty));
        ...
        <tr style="font-weight: bold;background-color: deeppink;color: white;">
            ...
            <tr>
                ...
                <tr>
                    ...
                    <?php for ($y = 0; $y <= count($product_header) - 1; $y++): ?>
                        <?php
                        $issue_car_x = getIssueCarQtySum(date('Y-m-d', strtotime($from_date)), $product_header[$y],$line_value->user_id);
                        ...
                        ?>
                        <td style="text-align: center;padding: 8px;border: 1px solid grey;width: 10%"><?= ($qty-$issue_car_x)==0?'-':number_format(($qty-$issue_car_x), 0) ?></td>
                    <?php endfor; ?>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <?php for ($y = 0; $y <= count($product_header) - 1; $y++): ?>
                        <td style="text-align: center;padding: 8px;border: 1px solid grey;width: 10%"><?= $balance_out_total_qty[$y]==0?'-':number_format($balance_out_total_qty[$y], 0) ?></td>
                    <?php endfor; ?>
                </tr>
                <tr>
...
//            $qty = $model;
//        }
        $model2 = \common\models\SalePosCloseIssueCarQty::find()->where(['product_id' => $product_id,'user_id'=>$user_id])->andFilterWhere(['date(trans_date)'=>$trans_date])->sum('qty');
        if ($model2) {
            $qty = $model2;
        }
    }
